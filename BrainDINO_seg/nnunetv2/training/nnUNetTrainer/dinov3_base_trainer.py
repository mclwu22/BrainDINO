...
class dinov3Trainer(nnUNetTrainer):
    ...
    def build_network_architecture( 
        ...
        # Load checkpoint
        chkpt = torch.load(
            '/scr2/yl_li/dinov3/ct_model_vitb_batch_512_adapt_high_res/converted_ckpt/4999/model.pth',
            map_location='cpu'
        )
        # Load with strict=False so it won’t crash on mismatches
        missing, unexpected = model.load_state_dict(chkpt, strict=False)

        # Extract patch_size from arch_init_kwargs or use default (512 for 2D)
        patch_size = arch_init_kwargs.get('patch_size', [512, 512])
        if isinstance(patch_size, (tuple, list)):
            patch_size_val = patch_size[0] if len(patch_size) > 0 else 512
        else:
            patch_size_val = patch_size
        sam = _build_sam(model, patch_size_val * 4, num_output_channels)
        return sam
    ...
    def configure_optimizers(self):
        ...
        warmup_epochs = 300   # 你可以改成需要的 warmup epoch 数
        start_factor = 1e-3  # 学习率从 initial_lr * start_factor 开始
        # warmup_scheduler = LinearLR(
        #         optimizer, 
        #         start_factor=start_factor, 
        #         total_iters=warmup_epochs
        #     )
        poly_scheduler  = PolyLRScheduler(optimizer, self.initial_lr, self.num_epochs)
        # scheduler = SequentialLR(
        #         optimizer, 
        #         schedulers=[warmup_scheduler, poly_scheduler], 
        #         milestones=[warmup_epochs]
        #     )
        return optimizer, poly_scheduler
