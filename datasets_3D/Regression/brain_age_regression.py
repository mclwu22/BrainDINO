...
class BrainAgeRegressionSet(Dataset):
    """
    ...
    """
    def __init__(self, config, base_dir,train_ratio=1.0, flag='train',teacher=False):
        super().__init__()
        self.config = config
        self.flag = flag
        ...
        if flag == "train":
            ...
            if train_ratio < 1.0:
                ...
                print(f"[ADNI] Train ratio={train_ratio}, "
                      f"using {n_keep}/{total} samples")
        else:   # valid or test
            ...
            with open(csv_path, "r") as f:
                reader = csv.reader(f)
                for row in reader:
                    self.all_labels.append(float(row[0]))
            ...
            if flag == "valid":
                ...
                print(f"[Reg] {flag} class distribution: {dict(class_counts)}")

        assert len(self.all_images) > 0, "No samples found!"
        print(f"[Reg] {flag} total loaded: {len(self.all_images)} samples")
        print(
            "[DEBUG]",
            min(self.all_labels),
            max(self.all_labels),
            type(self.all_labels[0])
        )
