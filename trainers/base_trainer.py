...
class BaseTrainer(object,):
    ...
    def load_model_state_dict(self, load_from: str, strict=True):
        ...
        # else:
        result = self.model.load_state_dict(self.resume_checkpoint['state_dict'], strict=strict)
        if not strict and hasattr(result, 'unexpected_keys') and result.unexpected_keys:
            self.recorder.logger.warning(f"Loaded with strict=False. Unexpected keys: {result.unexpected_keys}")
        if not strict and hasattr(result, 'missing_keys') and result.missing_keys:
            self.recorder.logger.warning(f"Loaded with strict=False. Missing keys: {result.missing_keys}")
